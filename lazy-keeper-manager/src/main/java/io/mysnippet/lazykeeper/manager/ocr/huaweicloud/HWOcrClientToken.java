...
public class HWOcrClientToken {
  ...
  /**
   * Construct a token request object for accessing the service using token authentication. For
   * details, see the following document:
  ...
  /**
   * Obtain the token parameter. Note that this function aims to extract the token from the header
   * in the HTTP request response body. The parameter name is X-Subject-Token.
   *
  ...
  public HttpResponse RequestOcrServiceBase64(String uri, String imgPath, JSONObject params)
    ...
    if (uri == null | uri == "" | imgPath == null | imgPath == "")
      throw new IllegalArgumentException(
          "The parameter for the requestOcrServiceBase64 constructor cannot be empty.");
    ...
    if (token != "") {
      ...
      try {
        ...
        // 2. Pass the parameters of VAT Invoice OCR, invoke the service using the POST method, and
        // parse and output the recognition result.
